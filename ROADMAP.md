...
- Not done: `clickCount`/`button` modifiers beyond the basics are wired but untested against real double/triple-click handlers; no `modifiers` (ctrl/shift/alt) param on `click` yet.
- **Done: `type_text({ ref | selector, text, clear })`** — `Input.insertText` after a real click to focus. `clear` uses a platform-aware select-all (Meta on macOS, Ctrl elsewhere; Ctrl+A is "move to line start" on macOS and silently prepends instead of replacing). No `perKey` escape hatch yet for widgets that listen for raw key events rather than `input`.
- **Done: `press_key({ key, modifiers })`**, optionally focusing a `ref`/`selector` first.
- **Done: `scroll_to({ ref | selector })` / `scroll_by({ dx, dy })`** — `scroll_to` uses `scrollIntoView`, `scroll_by` a trusted `mouseWheel` event.
- **Done: `select_option({ ref | selector, value })`, `set_checkbox({ ref | selector, checked })`** — `set_checkbox` is idempotent (only clicks when the current state differs) and reports `changed` so a no-op is distinguishable from a blocked click.
- **Done: hit-testing.** Every coordinate-dispatching action runs `elementFromPoint` at its target first and returns `hitTest: "ok" | "blocked"` with the blocking element. The input still dispatches — the point is that a click eaten by an invisible overlay no longer reports bare success, which was the exact failure `mobius-dead-click` exists to diagnose.
...
A code/product audit after Stage I shipped. Bug fixes, not new capability; `PROTOCOL_VERSION` moved to 3 for the last two.

- **Stable tab identity.** A tab's `clientId` used to be re-minted on every `webNavigation.onCommitted`, which invalidated whatever `tabId` the agent was holding and orphaned that tab's server buffer. It now outlives navigation and is retired only by `disableTab`. This is what made `wait_for_navigation` fire for full-page loads — the waiter was subscribed to the old id, so it had always timed out for anything but an SPA route change.
- **Unknown `tabId` is an error.** `resolveTabId` used to pass an unrecognised id through, so read tools returned `[]` — indistinguishable from "your app produced no errors."
- **Argument validation off the SDK path.** `run_sequence` and `control-request` called tool handlers directly, skipping the SDK's Zod validation *and* its defaults — so a `click` step without `button`/`clickCount` passed `undefined` to CDP, and `observe: {}` produced a 0ms window. `ToolDef` now carries a compiled parser both paths run first.
- **Keepalive.** An idle MV3 service worker is killed after 30s and nothing outside the browser can wake it, so a quiet tab silently stopped streaming until the `chrome.alarms` backstop fired up to a minute later. The server now sends an application-level `ping` every 20s while a client is connected (a protocol-level WebSocket ping frame is handled by the network stack and doesn't count as extension activity). The alarm remains as the cold-start backstop — it's the only thing that can start a dead worker.
- **Leader election on hub loss.** Followers used to poll `startWsServer` every 5s forever, constructing and discarding a `WebSocketServer` each time even while the hub was healthy. Promotion is now driven by the follower's own socket closing, with jitter so concurrent followers don't stampede. Promotion also re-opens persistence, which had been left closed since follower startup — the promoted hub was appending to a sink with no prune timer.
- **Cancelled jobs, job eviction, `getSince` cursor rewind, stale `lastDisconnectReason`.** Small correctness fixes; see the tests added alongside them.
- **Permissions.** `downloads` removed (the diagnostics export moves to the options page, which has a DOM and can drive its own anchor); `notifications` moved to `optional_permissions`, requested from its settings toggle rather than at install. `webNavigation` stays: `onCommitted` fires only on real document loads, while `tabs.onUpdated` also fires on History-API changes and would re-inject on every SPA route change.
- **Redaction defaults.** `capture-core`'s `DEFAULT_REDACTION` was entirely off, and duplicated the extension's stronger defaults. It's now the single source both use, and defaults on.

Deliberately **not** done here, tracked separately:

- **WebSocket authentication.** The hub accepts any local connection, and `control-request` dispatches every tool including `evaluate_js`. Origin-checking blocks the drive-by web-page case but not a local process; a token file the extension can't read isn't reachable without a paste step that hurts the zero-config flow this server is built around. Needs its own design pass, weighed against not breaking agent-initiated `open_tab`/`enable_capture`.
- **Unforgeable capture.** A page can suppress or fabricate events, because capture patches `console.*`/`fetch` inside the page's own realm — which is what makes it work. No in-realm shared secret fixes this: `postMessage` is observable by page scripts, so any nonce is readable in transit. The real fix is sourcing console/network from CDP (`Runtime.consoleAPICalled`, `Network.*`), which is browser-sourced and unforgeable, at the cost of a permanent debugger banner and different console fidelity. For now the server tells agents in its MCP instructions to treat captured page content as untrusted data.
