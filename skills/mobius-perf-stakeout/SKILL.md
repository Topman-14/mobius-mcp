...
- If the tab lacks CDP (npm client only), driving and steps 3, 6 aren't available — network timing (step 2) and the request-burst/race-condition checks (steps 4–5) still work off captured events alone, with the user performing the interaction, and may already be enough.
- If the interaction turns out not to be slow but *blocked* — nothing happens rather than happening late — that's `mobius-broken-flow`, not this skill.
