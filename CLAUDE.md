...
skills/                   Scenario-focused agent skills, one per use case (broken flow, walkthrough,
                           repro, perf stakeout, page audit) — also generated into MCP prompts
```
...
1. **`apps/browser-extension/injected.ts`** runs in the page's MAIN world. It calls `startCapture()` (`packages/capture-core/src/index.ts`), which monkey-patches `console.*`, `window.fetch`/`XMLHttpRequest`, `history.pushState`/`replaceState`, and (opt-in) a `MutationObserver`. Captured events are relayed out via `window.postMessage`. It also mounts `apps/browser-extension/src/modules/overlay/` (lazily, on first use) — the driving cursor/HUD visual feedback, exposed as `window.__mobiusOverlay` — plus `apps/browser-extension/src/lib/actions/`, exposed as `window.__mobiusActions`. `background.ts`'s action commands drive both over CDP `Runtime.evaluate`: `prepareTarget` resolves a `ref`/selector to viewport coordinates, moves the cursor, hit-tests the point, and hands the coordinates back for a trusted `Input.dispatch*` event.
...
Auto-enable per site is rule-based (`src/lib/rules.ts`), evaluated on `chrome.webNavigation.onCommitted` — capture is opt-in per tab otherwise (popup toggle). `onCommitted` is deliberate: it fires only for real document loads, where re-injection is needed. `chrome.tabs.onUpdated` also fires on History-API URL changes, so using it would re-inject (and double-patch) on every SPA route change.

A tab's `clientId` is minted once by `enableTab` and reused for the life of that tab's capture — it survives navigation, and only `disableTab` retires it. Agents hold it as `tabId`, so re-minting it on reload would invalidate their handle and orphan the server-side buffer.
...
- **A package's `src/` separates types, static data, and logic.** `types.ts` holds every type/interface/enum (especially ones reused across files, e.g. `Emit`, `RedactionOptions`); `data.ts` holds static constants/config objects derived from those types (e.g. `PROTOCOL_VERSION`, `DEFAULT_REDACTION`); generic helper functions move to a `utils/` folder, split into files by what they operate on or what they import (e.g. `packages/capture-core/src/utils/{stringify,headers,body,dom,redact,protocol}.ts`) rather than dumped alongside the package's primary exported functions or piled into one large `utils.ts`. See `packages/capture-core/src/` for the reference layout. This split applies in every workspace, not just `capture-core` — e.g. `apps/browser-extension/src/modules/overlay/{types,data}.ts` follow it too — so when adding a module, put types in `types.ts`, constants in `data.ts`, and pure helpers in `utils/`, rather than inlining them next to the logic that uses them.
- **Tests live in a workspace's top-level `tests/`, never beside the source.** `apps/mcp-server/tests/`, `apps/browser-extension/tests/`, `packages/capture-core/tests/` — mirroring the source folder structure below that where there's more than a handful. Every workspace with tests defines its own `test` script, since CI runs `npm run test --workspaces --if-present` and silently skips any workspace that doesn't.
- **`apps/browser-extension` keeps nothing but entry points at its root.** Only `injected.ts` (the MAIN-world entry) and the build configs live at the top level; everything else is under `src/`. Feature areas are folders in `src/modules/` (`overlay`, `snapshot`, `logs`, `options`, `popup`), and shared helpers are in `src/lib/` (`actions/`, `cdp.ts`, `rules.ts`, …). Don't add a new top-level folder for a feature — it goes in `src/modules/`.
- **`apps/mcp-server/src`'s three-folder split is load-bearing, mirror it rather than inventing a new shape.** `services/` holds the stateful singletons `index.ts` instantiates (`store`, `registry`, `commandDispatcher`, `jobs`, `debugSession`, `controlClient`, `persistence`, `diagnostics`) — each owns its own state and is constructed once. `transports/` holds only the two protocol-facing servers (`wsServer`, `mcpServer`) that talk to the outside world. `utils/` holds pure, stateless helpers grouped by what they operate on (`events`, `wait-for`, `har`, `tools`, `errors`). A new stateful piece goes in `services/`; a new protocol-facing entry point goes in `transports/`; a new pure helper goes in `utils/` — don't add a fourth top-level folder or a flat file back at `src/` for something that fits one of these three.
- **Write no comments.** Hard rule, not a default — see `~/Dev/CLAUDE.md`. Don't annotate a fix, justify a regex, or record what a bug was; that goes in the commit message. The few comments that survive here explain a hidden external constraint (e.g. `NOTIFICATION_ICON` in `background.ts`, why it's a PNG not SVG). If a test can pin the behaviour instead, write the test.
